...
percents=[10]  # [10] or [1] or [1,10] for both
        ...
        for percent in percents:
            ...
            for frame in frames:
                ...
                if do_tukey_test:
                    ...
                    mi_tukey = mi_tukey[mi_tukey.Percent == percent]
                ...
                if do_chi2:
                    ...
                    mi_chi = mi_chi[mi_chi.Percent == percent]
                ...
                for i in graph_n: #graph_n is just the conditions list in the current iteration
                    ...
                    if x_var == "Day":
                        if hue_split != "Condition":
                            ...
                            if plot_type == "stacked_bar":
                                ax_title = f'm{frame} {i} {percent}%_FBS'
                            elif plot_type == "line":
                                ax_title = f'm{frame} {i} vs {control_condition} {percent}%_FBS'
                            else:
                                ax_title = f'm{frame} {hue_choice[0]} vs {hue_choice[1]} {percent}%_FBS'
                    else:
                    ...
                    if plot_type == "violin":
                        if hue_split == "Condition":
                            violin_dir = os.path.join(
                                ...
                                f'{conditions[0]}_normalized', f'm{frame}', f'{percent}_%FBS'
                            )
                    ...
                    if boxplots:
                        box_dir = os.path.join(
                            ...
                            f'm{frame}', f'{percent}%_FBS'
                        )
                        if not os.path.exists(box_dir):
                            os.makedirs(box_dir)
                        percentFBS = '10%'
                        ax_title_boxplot = f'Day {box_day} Normalized Cell Counts Compared to {control_condition} - frame m{frame} - {percentFBS} FBS'
                    ...
                    if stackedbarplots:
                        stack_dir = os.path.join(
                            ...
                            f'{conditions[0]}_normalized', f'm{frame}', f'{percent}%_FBS'
                        )
                        if not os.path.exists(stack_dir):
                            os.makedirs(stack_dir)
                        percentFBS = '10%'
                        ax_title_stackedbar = f'Day {box_day} Cell Cycle Proportions Compared to {control_condition} - frame m{frame} - {percentFBS} FBS'
                    ...
                    if lineplots:
                        ...
                        percentFBS = '10%'
                        ax_title_line = f'Day {box_day} Normalized Cell Counts Compared to {control_condition} - frame m{frame} - {percentFBS} FBS'
                    ...
                    if colormap_plot:
                        ...
                        if do_wilcoxon_test:
                            ax_title_colormap_p = f'{control_condition} normalized m{frame} {percent}%_FBS Wilcoxon–Mann–Whitney test p-values'
                        else:
                            ax_title_colormap_p = f'{control_condition} normalized m{frame} {percent}%_FBS ttest p-values'

                        fname_colormap_p = os.path.join(colormap_dir, f'{ax_title_colormap_p}.png')
                        ax_title_colormap_c = f'{control_condition} normalized m{frame} {percent}%_FBS cohen\'s ds'
                        fname_colormap_c = os.path.join(colormap_dir, f'{ax_title_colormap_c}.png')
                        ax_title_colormap_m = f'{control_condition} normalized m{frame} {percent}%_FBS mean differences'
                        fname_colormap_m = os.path.join(colormap_dir, f'{ax_title_colormap_m}.png')
                        ax_title_colormap_anova = f'{control_condition} normalized m{frame} {percent}%_FBS anova p-values'
                ...
                # endregion
                    ...
                    if load_stats:  #  get data - to be implemented
                        ...
                        if do_ttest:
                            ...
                            if stats_var=='Total':
                                ...
                                mi_t = mi_t[mi_t['Percent'] == percent]
                        ...
                        if do_anova:
                            ...
                            mi_an = mi_an[mi_an.Percent == percent]
                        ...
                        #     mi_1 = mi[mi.Condition == control_condition]
                        #     mi_2 = mi[mi.Condition == i]
                        #     mi_tu = pd.concat([mi_1, mi_2])
                        #     mi_tu = mi_tu[mi_tu.Percent == percent]
                        #
                        #     if stats_var == 'Total':
                        #         mi_tu = mi_tu[mi_tu.Marker == marker_list[0]]
                        #
                        #         mi_tu['norm'] = 0.
                        #
                        #         mi_tu = mi_tu.reset_index(drop=True)
                        #
                        #         mi_tu_base = mi_tu[mi_tu['Day'] == start_day]
                        #         mi_tu_base = mi_tu_base.reset_index(drop=True)
                        #
                        #         for day in range(mi['Day'].min(), mi['Day'].max() + 1):
                        #             m_day = mi_tu[mi_tu['Day'] == day]
                        #             ind = m_day.index
                        #             m_day = m_day.reset_index(drop=True)
                        #             if normalization_type== 'relative':
                        #                 m_day['norm'] = (m_day[stats_var] - mi_tu_base[stats_var]) / mi_tu_base[stats_var]
                        #             else:
                        #                 m_day['norm'] = (m_day[stats_var]) / mi_tu_base[stats_var]
                        #             k = 0
                        #             for j in ind:
                        #                 mi_tu['norm'].iloc[j] = m_day['norm'].iat[k]
                        #                 k = k + 1
                        #
                        #     result_05 = statsmodels.stats.multicomp.pairwise_tukeyhsd(mi_tu[stats_var], mi_tu['Condition'],
                        #                                                               alpha=0.05)
                        #     result_01 = statsmodels.stats.multicomp.pairwise_tukeyhsd(mi_tu[stats_var], mi_tu['Condition'],
                        #                                                               alpha=0.01)
                        #     result_001 = statsmodels.stats.multicomp.pairwise_tukeyhsd(mi_tu[stats_var], mi_tu['Condition'],
                        #                                                                alpha=0.001)
                        #     # print(result.summary())
                        #     df_05 = pd.DataFrame(data=result_05._results_table.data[1:],
                        #                          columns=result_05._results_table.data[0])
                        ...
                        #                           columns=result_001._results_table.data[0])
                        #
                        #     # df_05.to_csv(path_or_buf=base_directory + '\\data\\tukey_results_05.csv',
                        #     #              index=None, header=True)
                        #     #
                        #     # df_01.to_csv(path_or_buf=base_directory + '\\data\\tukey_results_01.csv',
                        #     #              index=None, header=True)
                        #     #
                        #     # df_001.to_csv(path_or_buf=base_directory + '\\data\\tukey_results_001.csv',
                        #     #               index=None, header=True)
                        #
                    #endregion
                    ...
                    if plots:
                        ...
                        if individual_plots:
                            ...
                            # if plot_type=="violin":
                            #     if marker=="all":
                            #         ax = sns.catplot(x="Marker", y=stats_var,col=x_var, kind="violin",data=mi, hue=hue_split, hue_order=hue_choice,
                            #                         inner="quartiles", split="true", scale="area", linewidth=2,palette = sns.color_palette("hsv", 3),
                            #                         linestyles="-:")
                            #     else:
                            #         ax = sns.violinplot(x=x_var, y=stats_var, data=mi, hue=hue_split, hue_order=hue_choice,
                            #                         inner="quartiles", split="true", scale="area", linewidth=2,palette = sns.color_palette("hsv", 3),
                            #                         linestyles="-:")
                            #
                            # if plot_type == "box":
                            #     if marker == "all":
                            #         ax = sns.catplot(x="Marker", y=stats_var, col=x_var, kind="box", data=mi, hue=hue_split,
                            #                          hue_order=hue_choice,linewidth=2,
                            #                          palette=sns.color_palette("hsv", 3))
                            #     else:
                            #         ax = sns.boxplot(x=x_var, y=stats_var, data=mi, hue=hue_split, hue_order=hue_choice,
                            #                         linewidth=2)
                            #     # ax = sns.swarmplot(x="Condition", y=stats_var, data=mi)
                            ...
                            if plot_type=="stacked_bar":
                                ...
                                #print(means.Cell_percent)
                                #print(means.Cell_percent.unstack())
                            ...
                            if plot_type=="line":
                                ...
                                mi_new = mi_new[mi_new['Percent'] == percent]
                ...
                if save_excel_stats:
                    if do_ttest:
                        if plot_type=="stacked_bar":
                            stats_dir= os.path.join(
                                ...
                                'percent_cell', f'm{frame}', f'{percent}%_FBS'
                            )
                            if not os.path.exists(stats_dir):
                                os.makedirs(stats_dir)
                            for n in range(0,3):
                                ...
                                ttest_pvalues.to_csv(
                                    path_or_buf=os.path.join(stats_dir, f'ttests_pvalues_m{frame}_{markers[n]}_{percent}%_FBS.csv'),
                                    index=None, header=True)
                                ttest_cohens.to_csv(
                                    path_or_buf=os.path.join(stats_dir, f'ttests_cohens_m{frame}_{markers[n]}_{percent}%_FBS.csv'),
                                    index=None, header=True)
                                ttest_means.to_csv(
                                    path_or_buf=os.path.join(stats_dir, f'ttests_means_m{frame}_{markers[n]}_{percent}%_FBS.csv'),
                                    index=None, header=True)

                        if plot_type == "line":
                            stats_dir=os.path.join(
                                base_directory, 'stats', f'{conditions[0]}_normalized', 
                                'total_cell_n', f'm{frame}',  f'{percent}%_FBS'
                            )
                            if not os.path.exists(stats_dir):
                                os.makedirs(stats_dir)
                            data_t_p=tt_all[0]
                            ...
                            ttest_cohens = pd.DataFrame(data=data_t_c, index=days, columns=conditions)
                            ttest_means = pd.DataFrame(data=data_t_m, index=days,columns=conditions)
                            ttest_pvalues.to_csv(path_or_buf=os.path.join(stats_dir, f'ttests_pvalues_m{frame}_{percent}_FBS.csv'), index=None,header=True)  # Don't forget to add '.csv' at the end of the path
                            ttest_cohens.to_csv(path_or_buf=os.path.join(stats_dir, f'ttests_cohens_m{frame}_{percent}_FBS.csv'), index=None,header=True)  # Don't forget to a' at the end of the path
                            ttest_means.to_csv(path_or_buf=os.path.join(stats_dir, f'ttests_means_m{frame}_{percent}_FBS.csv'), index=None,header=True)  # Don't forget to a
                    ...
                    if do_anova:
                        ...
                        dir_fname= os.path.join(
                            stats_dir, f'anova_table_m{frame}_{percent}%_FBS.xls'
                        )
                ...
                if boxplots:
                    ...
                    if stats_var=='Total':
                        mi_box=mi
                        mi_box = mi_box[mi_box['Percent'] == percent]
                ...
                if stackedbarplots:
                    ...
                    if stats_var == 'Cell_percent':
                        ...
                        mi_new2 = mi_new2[mi_new2['Percent'] == percent]
