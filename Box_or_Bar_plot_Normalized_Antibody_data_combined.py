...
drug_order=['G', 'F', 'E', 'B', 'C', 'D']    # order of rows by which to plot, Control must be first!, e.g. ['E', 'C', 'D', 'B', 'F', 'G']
    ...
    for date in dates:
        ...
        for file in filelist:
            ...
            if date==dates[0] and file==filelist[0]:
                mi_box=mi_inter.copy()
            else:
                mi_box=mi_box.append(mi_inter)

    print('saving....',mi_box.shape)
    mi_box.to_csv(path_or_buf=base_directory + '//Combined_AntibodyResults_Column_means.csv',  index=None, header=True)
else:
    mi_box=  pd.read_csv(base_directory+'//Combined_AntibodyResults_Column_means.csv')


# mi_box['date'] = mi_box['date'].astype(str)
mi_box=mi_box.loc[mi_box.date==dates[0]]

conditions=['NS1643_20uM_Pantoprazole_100uM','Pantoprazole_100uM','NS1643_20uM']
mi_box=mi_box.loc[mi_box['Drug_Name'].isin(conditions)]
