...
for input_dir in dir_list:
    ...
    save_dir = input_dir# you can just write this as << save_dir = input_dir_base  >> if you want to save it in the same base directory as the input files, otherweise specify other directory

    input_fname_GFAP="/GFAP.csv" # needs "/" in front
    input_fname_nuclei="/Nuclei.csv" # needs "/" in front

    df=pd.read_csv(input_dir_base+input_fname_GFAP)
    ...
    mi.to_csv(path_or_buf=input_dir_base + '_' + str(threshold_value) + '_Thresholded_AntibodyResults.csv',  index=None, header=True)

    means_frame.to_csv(path_or_buf=input_dir + '_' + str(threshold_value) + '_Thresholded_AntibodyResults_Frame_means.csv',  index=None, header=True)
    means_columns.to_csv(path_or_buf=input_dir + '_' + str(threshold_value) + '_Thresholded_AntibodyResults_Column_means.csv',  index=None, header=True)
    means_rows.to_csv(path_or_buf=input_dir + '_' + str(threshold_value) + '_Thresholded_AntibodyResults_Row_means.csv',  index=None, header=True)

    stds_frame.to_csv(path_or_buf=input_dir + '_' + str(threshold_value) + '_Thresholded_AntibodyResults_Frame_stds.csv',  index=None, header=True)
    stds_columns.to_csv(path_or_buf=input_dir + '_' + str(threshold_value) + '_Thresholded_AntibodyResults_Column_stds.csv',  index=None, header=True)
    stds_rows.to_csv(path_or_buf=input_dir + '_' + str(threshold_value) + '_Thresholded_AntibodyResults_Row_stds.csv',  index=None, header=True)
