...
def export_prism_data(m_all_filepath,repeat):
    ...
    if repeat=='biological':
        mi_all = mi_all.groupby(['Day', 'Condition', 'Date', 'Marker'], sort=False, as_index=False).mean()
        date_list='all'
    if repeat == 'technical':
        mi_all = mi_all.groupby(['Day', 'Condition', 'Date', 'PlateNum', 'Marker', 'WellNum'], sort=False, as_index=False).mean()
    ...
    for date in date_list:
        ...
        for marker in ['RFP','YFP','Overlap']:

            mi_marker=mi_date.loc[mi_date.Marker==marker]

            for norm_colname in colnames:

                for day in days:
                    mi_day = mi_marker[mi_marker.Day == day]
                    ...
                    if repeat=='technical':
                        m_all_filepath_save = m_all_filepath_save.replace('frame_mean','Prism_'+ date + '_day_'+str(day) + '_marker_' + marker+'_mean')
                    else:
                        m_all_filepath_save=m_all_filepath_save.replace('frame_mean','Prism_day_'+str(day) + '_marker_' + marker+'_mean')
