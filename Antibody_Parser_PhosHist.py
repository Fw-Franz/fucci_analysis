...
subdir_pos=input_dir_base.rfind('/')
subdir=input_dir_base[subdir_pos:]
#endregion
start_time = time.time()

input_fname_PhosHist="/GFAP.csv" # needs "/" in front
input_fname_nuclei="/Nuclei.csv" # needs "/" in front
input_fname_positive_cells="/PositiveCells.csv" # needs "/" in front

print('loading data ... ')
df=pd.read_csv(input_dir_base+input_fname_PhosHist)
df_nuclei=pd.read_csv(input_dir_base+input_fname_nuclei)
df_positive=pd.read_csv(input_dir_base+input_fname_positive_cells)
...
mi=df[['ImageNumber','ObjectNumber','FileName_DisplayImage','Intensity_IntegratedIntensity_OrigBlue',
           'Intensity_IntegratedIntensity_OrigGreen','AreaShape_Area']]
mi=mi.rename(columns={"Intensity_IntegratedIntensity_OrigBlue": "DAPI_int_I", 
                      "Intensity_IntegratedIntensity_OrigGreen": "FarRed_int_I", "AreaShape_Area": "FarRed_area"})
...
mi.to_csv(path_or_buf=save_dir + subdir + '_AntibodyResults.csv',  index=None, header=True)

means_frame.to_csv(path_or_buf=save_dir + subdir + '_AntibodyResults_Frame_means.csv',  index=None, header=True)
means_columns.to_csv(path_or_buf=save_dir + subdir + '_AntibodyResults_Column_means.csv',  index=None, header=True)
means_rows.to_csv(path_or_buf=save_dir + subdir + '_AntibodyResults_Row_means.csv',  index=None, header=True)

stds_frame.to_csv(path_or_buf=save_dir + subdir + '_AntibodyResults_Frame_stds.csv',  index=None, header=True)
stds_columns.to_csv(path_or_buf=save_dir + subdir + '_AntibodyResults_Column_stds.csv',  index=None, header=True)
stds_rows.to_csv(path_or_buf=save_dir + subdir + '_AntibodyResults_Row_stds.csv',  index=None, header=True)
